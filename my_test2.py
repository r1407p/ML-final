...
def build_model(pretrainewd_path: str,
    ...
    if pretrainewd_path != "":
        ckpt = torch.load(pretrainewd_path, map_location=torch.device('cpu'))
...
if __name__ == "__main__":
    ...
    image_folder = "nycu2023mlfinalproject/data/test/"
    ...
    for fi, f in enumerate(files):
        ...
        with torch.no_grad():
            ...
            # 顯示預測結果
            id_label.append((f[:-4],classes[preds[0, 0].item()]))
            #id.append(f[:-4])
            #label.append(classes[preds[0, 0].item()])
            #print(f"Image: {f}")
            #print("Predicted class:", preds[0, 0].item())  # 顯示預測的類別
            # print("Class name:", cls_folders[preds[0, 0].item()])  # 顯示預測的類別名稱
            #print("-" * 30)
            print(fi)
    sorted_id_label = sorted(id_label, key = lambda x:x[0])
    ...
    # 寫入排序後的預測結果
        for id, label in sorted_id_label:
            writer.writerow([id, label])
